...
public class CollisionEvent_1 : MonoBehaviour
{
    // 衝突用
    ...
    void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject == PingpongRacket)
        {
            ...
            MainEvent_1.begin = true;
            Another.GetComponent<Rigidbody>().useGravity = true;

            if(Another.GetComponent<CollisionEvent_2>().miss == true)
            {
                Respawn(Another);
                Another.GetComponent<CollisionEvent_2>().miss = false;
            }
